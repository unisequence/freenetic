...
FNC_STAGED="/usr/bin/.fnc.freenetic.$$"
cp "$TMP_DIR/$FNC_BIN" "$FNC_STAGED" || fail "cannot copy fnc to /usr/bin"
chmod 0755 "$FNC_STAGED" || fail "cannot make fnc executable"
mv -f "$FNC_STAGED" /usr/bin/fnc || fail "cannot activate /usr/bin/fnc"
FNC_STAGED=""

if ! /usr/bin/fnc show version >/dev/null 2>&1; then
	fail "fnc was installed but could not start; check OpenWrt runtime libraries"
fi

info "installed Freenetic $RELEASE_TAG for $target"
info "fnc is available as /usr/bin/fnc"
